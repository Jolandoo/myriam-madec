...
export default async function TarifsPage(
  ...
  const sanityTarifs = await getTarifs(locale)
      ...
      <div className="w-full max-w-[1280px] mx-auto px-10 md:px-16 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            <h2 className="font-[var(--font-serif)] text-[var(--text-primary)] text-2xl">{t('priceByDuration')}</h2>
            <div className="rounded-[var(--radius-card)] overflow-hidden border border-[var(--gray-100)]" style={{ boxShadow: 'var(--shadow-card)' }}>
              {PRIX.map(({ duree, prix }, i) => (
                <div key={duree} className={['flex items-center justify-between px-6 py-4', i < PRIX.length - 1 ? 'border-b border-[var(--gray-100)]' : '', i % 2 === 0 ? 'bg-[var(--white)]' : 'bg-[var(--off-white)]'].join(' ')}>
                  <span className="font-[var(--font-sans)] text-[var(--text-primary)] font-medium">{duree}</span>
                  <span className="font-[var(--font-serif)] text-[var(--primary)] text-xl font-normal">{prix}</span>
                </div>
              ))}
            </div>
            <p className="font-[var(--font-sans)] text-[var(--text-muted)] text-sm">{t('groupMinimum')}</p>

            <div className="mt-4">
              <h2 className="font-[var(--font-serif)] text-[var(--text-primary)] text-2xl mb-4">{t('cancellationTitle')}</h2>
              <div className="flex flex-col gap-3">
                {ANNULATION.map(({ delai, montant }) => (
                  <div key={delai} className="flex items-center gap-4 p-4 rounded-xl bg-[var(--off-white)] border border-[var(--gray-100)]">
          ...
          <div className="flex flex-col gap-6">
            <div className="p-6 rounded-[var(--radius-card)] bg-[var(--primary-light)] border border-[var(--gray-100)] flex flex-col gap-4">
              <h3 className="font-[var(--font-serif)] text-[var(--text-primary)] text-xl">{t('conditionsTitle')}</h3>
              <ul className="flex flex-col gap-3">
                {CONDITIONS.map((cond) => (
                  <li key={cond} className="flex items-start gap-3">
                    <Check size={15} className="text-[var(--primary)] shrink-0 mt-0.5" />
                    <span className="font-[var(--font-sans)] text-sm text-[var(--text-primary)] leading-relaxed">{cond}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="p-6 rounded-[var(--radius-card)] bg-[var(--primary)] flex flex-col gap-3 text-center" style={{ boxShadow: 'var(--shadow-card)' }}>
              <h3 className="font-[var(--font-serif)] text-white text-xl">{t('quoteTitle')}</h3>
              <p className="font-[var(--font-sans)] text-white/75 text-sm leading-relaxed">{t('quoteDesc')}</p>
              <Link href="/contact" className={['mt-2 inline-flex items-center justify-center px-6 py-3 rounded-[var(--radius-btn)]', 'bg-white text-[var(--primary)]', 'font-[var(--font-sans)] font-semibold text-sm', 'hover:bg-white/90 transition-colors duration-200'].join(' ')}>
                {t('contactMe')}
              </Link>
            </div>
          </div>
